...
BNOctaves::Context* m_BNOctaves = nullptr;
...
int main(int, char**)
{
    ...
    ::RegisterClassExW(&wc);
    HWND hwnd = ::CreateWindowW(wc.lpszClassName, L"BNOctaves", WS_OVERLAPPEDWINDOW, 100, 100, 1280, 800, nullptr, nullptr, wc.hInstance, nullptr);
    ...
    BNOctaves::Context::LogFn = &LogFunction;
    BNOctaves::Context::PerfEventBeginFn = &PerfEventBeginFn;
    BNOctaves::Context::PerfEventEndFn = &PerfEventEndFn;
    m_BNOctaves = BNOctaves::CreateContext(g_pd3dDevice);
    if (!m_BNOctaves)
    {
        printf("Could not create m_BNOctaves context");
        return 1;
    }
    // Gigi Modification End
    ...
    while (!done)
    {
        ...
        if (m_BNOctaves && ImGui::CollapsingHeader("BNOctaves"))
            BNOctaves::MakeUI(m_BNOctaves, g_pd3dCommandQueue);
        // Gigi Modification End
        ...
        // Gigi Modification Begin - OnNewFrame and Execute
        {
            ...
            BNOctaves::OnNewFrame(NUM_FRAMES_IN_FLIGHT);
            if (m_BNOctaves)
                BNOctaves::Execute(m_BNOctaves, g_pd3dDevice, g_pd3dCommandList);
    ...
    if (m_BNOctaves)
    {
        BNOctaves::DestroyContext(m_BNOctaves);
        m_BNOctaves = nullptr;
    }
    // Gigi Modification End
